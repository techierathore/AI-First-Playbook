...
| Still prompted for permission in YOLO mode | `PLAYBOOK_YOLO=1` is not reaching the harness process (WSL → Windows binary needs `WSLENV=$WSLENV:PLAYBOOK_YOLO`), or Claude Code has a `permissions.ask` rule for that tool — hook allows never override `ask`/`deny` rules. See [YOLO-Mode-Guide.md](YOLO-Mode-Guide.md) §8. |
