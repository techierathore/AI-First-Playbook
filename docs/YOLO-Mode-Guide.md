...
|---|---|
| The harness stops for permission (delete a folder, run a command, read git) even though you said "just do it" | Every permission prompt is auto-approved **mechanically** — by a Claude Code hook / an OpenCode plugin — except git history writes, which stay denied |
...
| A Claude Code `/goal` | The command bodies and `AGENTS.md` | Prompt-level gates only |
...
```bash
# One phase, unattended, with limit handling (Claude Code)
node scripts/playbook-yolo.mjs --harness=claude-code --cwd=/path/to/repo \
     --prompt "/implement docs/Cost/App-Cost-FullStack-Implementation-Checklist.md"

# End-to-end goal: implement → verify → fix → verify … until every item PASSes (OpenCode)
...
PLAYBOOK_YOLO=1 claude              # Claude Code: yolo-hook.mjs allows every PreToolUse / PermissionRequest
```
...
Subscription plans on both harnesses enforce a 5-hour window and a weekly cap; the API
...
3. **Adds the buffer** — 15 minutes by default (`--buffer=<min>`) — and prints
   ...
   after a reboot.
4. **Resumes the same session** (`claude --resume <id>` / `opencode run --session <id>`)
...
## 6. What ships, by harness

| | OpenCode | Claude Code |
|---|---|---|
| Policy (single source) | `harness/opencode/plugin/yolo-policy.mjs` | same file, copied to `.claude/hooks/yolo-policy.mjs` by the pack generator |
| Permission bypass | `plugin/yolo.ts` → `permission.ask` sets `allow` / `deny`; `tool.execute.before` throws on git writes even when the agent's config already says `bash: allow` | `hooks/yolo-hook.mjs` on **PreToolUse** (`permissionDecision: allow`, exit 2 to block git writes) and **PermissionRequest** (`decision: allow/deny`) |
| Limit detection | `event` → `session.error` → `verification/yolo/rate-limit.json` | supervisor parses the `-p --output-format json` result text |
| Launch flags used by the supervisor | `opencode run --auto --format json [--agent …] [--session …]` | `claude -p --output-format json --permission-mode bypassPermissions [--resume …]` |
| Registration | `opencode.json` → `"plugin": [spec-guardrails.ts, yolo.ts]` (order matters: guardrail first) | `.claude/settings.json` → both hooks, generated |

Both carriers are **inert without `PLAYBOOK_YOLO=1`** — an ordinary interactive session is
unchanged.

### Claude Code specifics

- `permissions.deny` and `permissions.ask` rules in `settings.json` are evaluated *regardless*
  of a hook's allow. A YOLO install must not carry `ask` rules for the tools it wants
  unattended; `deny` rules are fine (they only make things stricter).
- `--permission-mode bypassPermissions` is refused in some environments (e.g. running as
  root without a sandbox flag). Use `--permission-mode=auto` then — the hook's allow
  answers the classifier's prompts.
- `/goal` does **not** change the permission mode; it only keeps the turn loop going. The
  supervisor pairs it with the hook so both halves are covered. Add `--claude-goal` to send
  the goal through `/goal` as well as the playbook prompt.
...
| Claude Code hook (`yolo-hook.mjs`) | Driven by hand over stdin: `rm -rf build` → `permissionDecision: allow`; `git commit` → exit 2; PermissionRequest `git push` → `decision: deny`; no-op without the env var |
| Supervisor command lines | `--dry-run` for both harnesses |
| Pack generation, plugin registration order, policy drift | `node scripts/playbook-validate.mjs` |
...
Two Claude Code specifics worth repeating because they are the likeliest first-run
surprises: `permissions.ask` / `deny` rules in `settings.json` are evaluated **regardless**
of the hook's allow (remove `ask` rules for tools you want unattended), and if
`--permission-mode bypassPermissions` is refused on your VM, run the supervisor with
`--permission-mode=auto`.

If something in this table turns out wrong in practice, the fix belongs in
`yolo-policy.mjs` (parsing / allow-list) or the carrier for that harness — then update this
...
| Symptom | Action |
|---|---|
| Still being prompted in the TUI | `PLAYBOOK_YOLO` is not reaching the process. From WSL launching a Windows binary: `export WSLENV=$WSLENV:PLAYBOOK_YOLO`. Claude Code: confirm `.claude/settings.json` registers `yolo-hook.mjs` under both `PreToolUse` and `PermissionRequest`, and that no `permissions.ask` rule covers the tool. |
...
| `bypassPermissions` refused | Run the supervisor with `--permission-mode=auto`. |
| Test the policy by hand | `echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"git commit -m x"}}' \| PLAYBOOK_YOLO=1 node .claude/hooks/yolo-hook.mjs` must exit 2; with `rm -rf build` it must print `"permissionDecision":"allow"`. |
