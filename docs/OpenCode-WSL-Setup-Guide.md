...
In the project you're working on, create `.opencode/config.json`:
...
API keys are entered directly. A Claude Pro/Max subscription login prints a URL — open it in
your Windows browser and paste the code back.
...
git clone <your-org-git-url>/AI-First-Playbook
git clone <your-org-git-url>/MyWebApp
cd MyWebApp
```

Install the Playbook harness:
...
```bash
mkdir -p playbook
cp ~/work/AI-First-Playbook/playbook/environment-profile.yml playbook/
...
Add an `AGENTS.md` at the repo root from
`~/work/AI-First-Playbook/templates/agents-md-template.md`.

Full details: `harness/README.md`.
...
`Ctrl+Shift+P → WSL: Connect to WSL`, and open `~/work/MyWebApp`. Run `opencode` in the
...
```powershell
function oc { wsl -d Ubuntu-24.04 -e bash -lc "cd ~/work && opencode $args" }
```
...
$menus = @(
  ...
  @{ Name = "OpenOpenCodeHere"; Label = "Open OpenCode here"; Run = "opencode; exec bash" }
)
...
> own **tab separator**, so `-lc "opencode; exec bash"` becomes *"run opencode"* in one tab
> and *"run exec bash"* in another — and the second has no such program, hence the error.
>
...
> wt.exe -d "%V" -- "C:\Windows\System32\wsl.exe" -d Ubuntu-24.04 --cd "%V" -e bash -lc "opencode\; exec bash"
> ```
>
...
Once you have done Step 4 by hand at least once and understand what it does, the repository
has a script that performs steps 4, 5, 6 and 7 together. Useful for setting up a second
machine or a teammate's.
...
isn't loading — check `.opencode/plugin/` exists and that you ran the `npm install` in
Step 11.
...
The Docker approach (`docs/OpenCode-Setup-Guide.md`) is superseded. Keep it only for CI images.
...
The reasoning behind these choices, with sources, is in
[OpenCode-Guide.md](OpenCode-Guide.md).
