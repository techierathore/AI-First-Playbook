...
## 0. Requirement updates adopted from the shipped solo edition (2026-08-28)

Seven things the TechieFlow implementation settled that changed the original design. All seven are reflected in the implementation described here.

**0.1 — `why_missed` shipped, and the port back was not verbatim.** SCHEMA §5.5.6 carries the Playbook's six values **plus a seventh, `instruction-ignored`** — a written rule existed, in a file the agent had loaded, and was not honoured. The Playbook adopted the seventh value with an explicit **agent-origin-only** definition; it may never classify a human's behaviour (Decision Log, 2026-08-29).

- The value earns its keep because an agent skipping a loaded step is a distinct failure mode. A team also has a human process-failure case, and the implementation deliberately does not collapse that into this label; doing so would create a performance judgement about a named `actor` (see §6.1).
- The other six values remain identical across editions, and the agent-only restriction keeps the seventh comparable without turning it into a person-facing metric.

**0.2 — `why_missed` is optional, and that has a reporting consequence this document did not state.** `null` means *not assessed*, never a zero in some category, so **every distribution over the field is denominated on records that carry it** and prints `n of N assessed`. Validation also surfaces `escapes_missing_why` as `n of N eligible escapes` — escapes that arrived with no `why_missed`, excluding records before the field's `FIELD_SINCE` date. These are the most valuable eligible records in the stream arriving incomplete. Phase 9 already produces this judgement in prose, so measure completeness rather than assume it.

**0.3 — Confidence is derived, not authored.** §2.2's third category lists the agent-authored fields; `origin_confidence` is **not** one of them, and neither is `origin_model`. The emitter resolves both from the run record and **forces the model to `null` when the lookup fails, ignoring caller-supplied provenance.** The honesty rule in §2.2 stands — *an agent may classify, but may never report a number or a provenance verdict*.
...
A deliberately-abandoned defect is not backlog, but the next `FAIL` on that item is still the *same* defect and must not open a second record. `deferred` is outstanding work and stays open in both. The implementation maps this onto `templates/checklist-metadata.yml`'s status vocabulary (`pass` · `fail` · `data-gap` · `blocked` · `deferred` · `abandoned`) and comments the divergence in code so a reviewer does not "fix" the two to agree and break one of them.

**0.5 — Add the Playbook's equivalent of `*log-miss`, and treat it as load-bearing rather than a convenience.** `/log-miss` is the 20-second front door: it classifies a one-sentence report, writes the record and checklist line, and **never boots the app, never reproduces, never touches `src/`**. It wraps `playbook-miss.mjs open`, including `--fixed` for a miss reported after repair. This gives review, standup and between-phase discoveries a durable home without making reproduction a precondition for recording.

**0.55 — Adopt a third record kind, `miss-amend`, from day one — the two-kind design had a hole.** TechieFlow shipped with `miss` and `miss-fix` only, and the first outside user hit the gap within a day (TfLens TF-005): the append-only doctrine says *"if a record is wrong, the correction is a new record, never an edit"*, and neither kind could carry a correction. A field left `null` — most sharply a `why_missed` on a record written before the field existed — was otherwise **unreachable**. The Playbook implementation includes `miss-amend` from its first schema.
...
That analysis now has a durable counterpart: the checklist prose remains human-readable while the append-only stream can be counted, grouped by phase/model when linked, and joined to measured fix cost.
...
**Do not do that.** The pre-implementation Telemetry Guide recommended ignoring `verification/telemetry/` as capture noise. A miss record is the opposite kind of object: it is the durable finding, it outlives the run that produced it, and it is the thing a team looks back at in three months. The guide now ignores only the exact `events.ndjson` path; putting misses in the transient stream would destroy the data by design.

**Decision: a second, durable file — `verification/telemetry/misses.ndjson`.**

- **Committed**, and explicitly carved out of the old "gitignore `verification/telemetry/`" advice. The Telemetry Guide now names the two files separately rather than the directory.
...
*(Implemented as three kinds from day one: `miss`, `miss-fix`, and `miss-amend`.)*
...
| **`why_missed`** | `missing-checklist-item` · `insufficient-verify-method` · `code-audit-limitation` · `ambiguous-acceptance` · `dependency-not-declared` · `instruction-ignored` (agent-origin only) · `other`. **Optional — `null` is *not assessed*, so distributions are denominated on records that carry it (§0.2).** |
...
The join is phase-aware rather than session-only. Framework phases map to slash commands
(`plan` → `feature-plan`; `build`/`self-review` → `implement`; verification phases → `verify`;
`fix` → `fix`; post-verification/production bugs → `analyze-fix`), and the latest matching window
starting at or before the close timestamp wins. Thus reused sessions and repeated commands do not
drift to an old window. `shared:<n>` groups only records that resolve to the same exact window.
Only usage and child token/cost values are divided; `subagents.count` remains the integer child
session count observed for that window.
...
| Record kinds | `miss` · `miss-fix` · `miss-amend` (§0.55) | same three |
...
| Dollars | OpenCode only; `null` on Claude/Codex | OpenCode event windows capture provider cost; Claude classification without compatible events degrades to `null` |
| `why_missed` | **ported and shipped 2026-08-28** (SCHEMA §5.5.6), plus `instruction-ignored` | same seven; `instruction-ignored` is explicitly agent-origin only (§0.1) |
| Front door for a one-line report | `*log-miss` — shipped; no boot, no repro (§0.5) | `/log-miss` — same no-boot/no-repro boundary |
...
The port is **not verbatim in policy**. Both editions carry **`instruction-ignored`**, but the team edition restricts it to a written framework rule an agent had loaded and did not honour. It must never describe a human. The other six values are identical, so the field remains comparable without creating a person-facing performance label.
...
## 9. Implementation notes (2026-08-29)

- `playbook-miss.mjs` is the only append door. `open --if-new` uses the still-live predicate;
  `close` omits an unknown `fix_run_id`; `amend` only completes null `why_missed`; reads and
  refused writes print their outcome. Telemetry failures do not fail a delivery phase.
- `miss-lib.mjs` owns all closed vocabularies, schema validation, append-only amendment folding,
  `FIELD_SINCE`, assessed/escape completeness notes, backlog/live predicates and exact event
  window lookup. Invalid amend values are rejected during folding as well as reported by
  validation, so free text cannot leak into joined output. Caller-supplied provenance and numeric
  cost fields are not trusted.
- `playbook-telemetry.mjs --misses` folds valid amendments and enriches fixes at read time.
...
- Repository retention is intentionally asymmetric: ignore only
  `/verification/telemetry/events.ndjson`; commit and retain
  `verification/telemetry/misses.ndjson`. Broader telemetry/NDJSON ignore rules are invalid.
- The stream stores ids, paths and closed vocabularies only. Actor data is aggregate-only and
  every per-actor miss/rework/cost report is prohibited. Optional-field reporting uses `n of N
