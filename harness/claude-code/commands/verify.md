...
blocked at the tool level by `.claude/hooks/spec-guardrails-hook.mjs`.
...
never changes an item outcome or the verification verdict. Use `--harness=claude-code` and
