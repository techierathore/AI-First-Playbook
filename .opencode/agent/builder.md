---
description: >
  Wave worker for /implement and /fix. Implements exactly the checklist slice
  it is handed — nothing more. Runs at the standard tier regardless of the
  orchestrator's model, so parallel waves never silently inherit a frontier
  model. Spawned via the task tool; never used as a primary agent.
mode: subagent
temperature: 0.1
permission:
  edit: allow
